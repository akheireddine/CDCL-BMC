...
static void * mainThrSharing(void * arg)
{
   ...
   while (true) {
      ...
      log(1, "Sharer %d enter in round  %d, received cls %ld, shared cls %ld\n",
...
Sharer::Sharer(int id, int pin, SharingStrategy * sharingStrategy,
               vector<SolverInterface *> producers,
               vector<SolverInterface *> consumers)
{
   this->id              = id;
   ...
   sharer  = new Thread(pin, mainThrSharing, this);
}
