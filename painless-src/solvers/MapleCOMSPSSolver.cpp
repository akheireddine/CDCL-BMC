...
void cbkMapleBlackbox(void *issuer, MapleCOMSPS::vec<MapleCOMSPS::vec<MapleCOMSPS::Lit>> &out_cls)
{
   MapleCOMSPSSolver *mp = (MapleCOMSPSSolver *)issuer;
   mp->env_bmc->explore_automata_each_loop(out_cls);
}
...
void cbkMapleCOMSPSExportClause(void *issuer, int lbd, vec<Lit> &cls)
{
   ...
   ncls->lbd = cls.size(); // lbd;
   ncls->from = mp->id;

   mp->clausesToExport.addClause(ncls);
}

void cbkMapleCOMSPSExportTrail(void *issuer, int trail_sz, vec<int> &cls)
{
   MapleCOMSPSSolver *mp = (MapleCOMSPSSolver *)issuer;
   ...
   mp->trailToExport.addClause(ncls);
}

void cbkMapleCOMSPSTryExportTrail(void *issuer, int trail_sz, vec<int> &cls)
{
   MapleCOMSPSSolver *mp = (MapleCOMSPSSolver *)issuer;
   if(mp->env_bmc->new_trail!=NULL)
      return;
   ...
   ncls->from = -1;
   ncls->size = cls.size();
   if (!mp->env_bmc->tryLockTrail(ncls))
...
bool cbkMapleCOMSPSImportClauseReducer(void *issuer, int *lbd, vec<Lit> &mcls)
{
   MapleCOMSPSSolver *mp = (MapleCOMSPSSolver *)issuer;

   ClauseExchange *cls = NULL;

   while (mp->clausesToImport.getClause(&cls) != false && cls->from != 3)
      ; // should comes from REDUCER

   if (mp->clausesToImport.getClause(&cls) == false)
      return false;
   // printf("(CDCL) imported clause from reducer of sz %ld\n",mcls.size());
...
MapleCOMSPSSolver::MapleCOMSPSSolver(int id, EnvBMC *env) : SolverInterface(id, MAPLE)
{
   ...
   solver->exportClauseCallback = cbkMapleCOMSPSExportClause;
   solver->importClauseCallback = cbkMapleCOMSPSImportClause;
   solver->importUnitCallback = cbkMapleCOMSPSImportUnit;
   solver->issuer = this;
   ...
   if (env)
   {
      env->solver = solver;
      solver->bmc = env;
      solver->exploreAutomataCallback = cbkMapleBlackbox;
      if (env->oneTrailAtTime)
         solver->exportTrailCallback = cbkMapleCOMSPSTryExportTrail;
      else
         solver->exportTrailCallback = cbkMapleCOMSPSExportTrail;
   }
}

MapleCOMSPSSolver::MapleCOMSPSSolver(const MapleCOMSPSSolver &other, int id, EnvBMC *env) : SolverInterface(id, MAPLE)
{
   lbdLimit = Parameters::getIntParam("lbd-limit", 4);
...
void MapleCOMSPSSolver::getTrailClauses(vector<ClauseExchange *> &clauses)
{
   trailToExport.getClauses(clauses);
}
