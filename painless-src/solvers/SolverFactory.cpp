...
SolverInterface *
SolverFactory::createMapleCOMSPSSolver(EnvBMC * env_bmc)
{
   int id = currentIdSolver.fetch_add(1);

   SolverInterface * solver = new MapleCOMSPSSolver(id, (EnvBMC*)env_bmc);
...
EnvBMC* SolverFactory::createBlackboxBMC(){
   int id = currentIdSolver.fetch_add(1);
   // EnvBMC* bxbmc = new EnvBMC( id, Parameters::getBoolParam("inter"), Parameters::getBoolParam("loop"),Parameters::getBoolParam("edge"));
   EnvBMC* bxbmc = new EnvBMC(id);
   if(Parameters::getIntParam("c",1) > 1)
      bxbmc->seq = false;
   return bxbmc;
}
...
SolverFactory::createMapleCOMSPSSolvers(int maxSolvers,
                                        ...
                                        EnvBMC *env_bmc)
{
   solvers.push_back(createMapleCOMSPSSolver(env_bmc));
   ...
   for (int i = 1; i < maxSolvers; i++) {
      solvers.push_back(cloneSolver(solvers[0], env_bmc));
   }
}
                           ...
                           EnvBMC *env_bmc)
{
   ...
   switch(other->type) {
      case MAPLE :
	      solver = new MapleCOMSPSSolver((MapleCOMSPSSolver &) *other, id, env_bmc);
      	break;
